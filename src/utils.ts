...
export const copyFile = async (source: string, dest: string) => {
	...
	await new Promise<void>((resolve, reject) => fs.copyFile(source, dest, (err) => { err ? reject(err) : resolve() }))
}

export const writeFile = async (path: string, data: string) => {
	await new Promise<void>((resolve, reject) => fs.writeFile(path, data, (err) => { err ? reject(err) : resolve() }))
}

export const mkdir = async (path: string, options: fs.MakeDirectoryOptions = {}) => {
	await new Promise<void>((resolve, reject) => fs.mkdir(path, options, (err) => { err ? reject(err) : resolve() }))
}

export const rm = async (path: string, options: fs.RmOptions = {}) => {
	await new Promise<void>((resolve, reject) => fs.rm(path, options, (err) => { err ? reject(err) : resolve() }))
}

export const rmdir = async (path: string, options: fs.RmDirOptions = {}) => {
	await new Promise<void>((resolve, reject) => fs.rmdir(path, options, (err) => { err ? reject(err) : resolve() }))
}

export const readdir = async (path: string): Promise<string[]> => {
	return new Promise<string[]>((resolve, reject) => fs.readdir(path, (err, files) => { err ? reject(err) : resolve(files) }))
}
